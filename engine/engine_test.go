...
// 修复，一场景一函数（失败定位性）。行为级端到端回归（会话/审批/steering/
// usage 全链）在产品装配层测试（internal/agent）经 llmtest 假模型驱动；超长
// 工具结果截断/外置归 reduction_test.go（2026-08-26 newSpiller 退役，截断面
// 统一移交 reduction）。
...
// TestSanitizeHistoryFillsEmptyArgs 空 arguments 回灌 "{}"。
func TestSanitizeHistoryFillsEmptyArgs(t *testing.T) {
	msgs := []*schema.Message{
...
// TestSanitizeHistoryStripsDanglingToolCalls 悬空 tool_calls 剥离（无应答 tool
// 消息，文本保留）。
func TestSanitizeHistoryStripsDanglingToolCalls(t *testing.T) {
	msgs := []*schema.Message{
		schema.UserMessage("问题"),
...
// TestSanitizeHistoryDropsEmptyAssistant 空 assistant 剔除。
func TestSanitizeHistoryDropsEmptyAssistant(t *testing.T) {
	msgs := []*schema.Message{
		schema.UserMessage("问题"),
...
// TestSanitizeHistoryDropsOrphanToolMessage 孤儿 tool 消息剔除。
func TestSanitizeHistoryDropsOrphanToolMessage(t *testing.T) {
	msgs := []*schema.Message{
		schema.UserMessage("问题"),
