...
// 反斜杠形态在 Windows 宿主上命中）。大小写折叠匹配（安全审查 2026-09-06）：
// macOS/Windows 默认大小写不敏感文件系统上「Docs/」会落进 protect=["docs"]
// 的真实目录，字节级比较即旁路；大小写敏感文件系统上折叠只会多拦不会漏
// （收紧方向，可接受）。
func PathBlocked(rel string, dirs []string) bool {
	...
	for _, d := range dirs {
		if strings.EqualFold(p, d) ||
			(len(p) > len(d) && p[len(d)] == '/' && strings.EqualFold(p[:len(d)], d)) {
			return true
		}
	}
	return false
