...
| 厂家预设              | `BuiltinProviders` 内置 DeepSeek 官方推荐 openai 端点（api.deepseek.com + deepseek 方言）与智谱 GLM 端点（openai 协议 Chat Completion：GLM-5.3 纯文本 + GLM-5.3-Flash 多模态）；DeepSeek /anthropic 端点不预置——自定义 Kind=anthropic 可接（思考走协议原生预算档零方言）；`Resolve` 原样 / `ResolveMerged` by-ID 参数补全（用户显式值优先，密钥/启用权不受内置影响）                                                                                                                                                                                                |
