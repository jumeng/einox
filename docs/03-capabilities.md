...
| thinking 双协议映射    | effort 三档（low/high/max）：anthropic 协议 = 预算分档（BudgetTokens），openai 协议 = 思考方言（`dialect=deepseek` 发扩展字段 / `dialect=glm` 智谱同形私有块 / `dialect=effort` 通用 reasoning_effort / 空方言零思考字段）                                                                                                                                                                                                                     |
