...
- **模型面适配铁律**：多供应商 + 产品级 ⇒ 必须适配，没有第三条路（零适配/多供应商/产品级三角最多取二）。厂家私有知识只走三口收编——`Dialect`（转形：私有参数）、`ModelSpec` 能力元数据（对账：有无与档位，适配不无中生有）、`Classify`（错误归约），**绝不写进通用分支**；方言词汇表外的能力（Files/Batch/服务端状态）不进统一面。定案与例证见 [docs/03](docs/03-capabilities.md) 模型面
